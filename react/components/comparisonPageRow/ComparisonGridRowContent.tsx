...
const CSS_HANDLES = ['comparisonCol', 'comparisonColName', 'fieldNameCol', 'title']
...
const List = ({ children, comparisonProducts, field, specName, groupName }: Props) => {
  ...
  const newListContextValue = useMemo(() => {
    const componentList =
      ...
      comparisonProducts.map((comparisonProduct) => {
        ...
        return groupName ? ( <div className={`${cssHandles.title} pa5 b`}>
        <span>{groupName}</span>
      </div>) : specName ? ( 
            <div
  ...
  }, [comparisonProducts, cssHandles.comparisonCol, cssHandles.comparisonColName, cssHandles.fieldNameCol, cssHandles.title, field, groupName, list, specName])
...
const ComparisonGridRowContent = ({ children, field , specName, groupName}: Props) => {
